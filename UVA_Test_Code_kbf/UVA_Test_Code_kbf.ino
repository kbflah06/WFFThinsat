...
and convert it to uva intensity (uW/m^2), 
...
Data is two bytes in uW/m^2 appended with zeroes to make the packet to be 35 bytes for 
Serial data format. 
*/
...
float intensity_step = 25;     //intesity step count in uW/m^2
...
void loop() {
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    { 
      Serial.println(i);
      Serial.println(uva_intensity_mapping_table[i]);
  ...
  i = 4;      //testing the code for higher value of UVA level, greater than 2055 digital counts  
  
  word inten = word(intensity[i]);
  
  Serial.println("***********");
  Serial.println(inten, HEX);
  ...
  Serial.println("infoTx[6], HEX");
  
  Serial.println(infoTx[6], HEX);
  Serial.println(infoTx[6], 3);
  Serial.println(infoTx[7], HEX);
  Serial.println(infoTx[7], 3);
  
  Serial.print("intensity\t");
  Serial.println(intensity [i], HEX);
  Serial.print("temp_h \t");
  Serial.println(temp_h, 3);
  Serial.print("temp_l \t");
  Serial.println(temp_l, 3);
  
  
  Serial.print("uva_data_h \t");
  Serial.println(uva_data_h, HEX);
  Serial.print("uva_data_l \t");
  Serial.println(uva_data_l, HEX);
  Serial.println("uva_data, \t");
  
  Serial.println(uva_data_h, 3);

//  Serial.print("uva_data:  ");
//  Serial.println(uva_data);
//  Serial.println(uva_data, HEX);
